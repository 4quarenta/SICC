...
function AlertsView({ operator }: { operator: Operator }) {
  ...
  async function createAlert(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // O operador precisa apenas da confirmação operacional. O status técnico
      // do provedor permanece disponível no backend/logs, sem aparecer na UI.
      setNotice("QTC publicado.");
      await loadAlerts();
    } catch (error) {
